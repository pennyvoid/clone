<template>
  <div class="bg-white w-2/3 rounded shadow mt-6 overflow-hidden" v-if="posts">
    <div class="flex flex-col p-4">
      ...
      <div class="mt-4">{{ post.data.attributes.body }}</div>
    </div>
    <div class="w-full mt-4" v-if="post.data.attributes.image">
      <img :src="post.data.attributes.image" class="w-full" alt="post image" />
    </div>
    <div class="flex justify-between px-4 text-gray-600 text-sm mt-4">
      <div class="flex">
    ...
    <div class="flex justify-between items-center border-1 border-gray-400 m-4 h-12">
      <button
        class="hover:bg-gray-200 w-full flex justify-center items-center rounded-lg h-full focus:outline-none focus:shadow mr-2"
        :class="[
      ...
      <button
        class="hover:bg-gray-200 w-full flex items-center justify-center rounded-lg h-full focus:outline-none"
    ...
    <div v-if="comments" class="border-t border-gray-400 p-4 pt-2">
      <div class="flex mt-2">
        <input
          ...
          v-model="commentBody"
          name="comment"
          class="bg-gray-200 w-full h-10 rounded-full focus:outline-none pl-4"
        />
        <button
          v-if="commentBody"
          class="bg-gray-400 ml-2 px-2 py-1 rounded-full focus:outline-none"
          @click="
      ...
      >
        <div class="w-10">
          <img
            ...
            alt
            class="w-12 h-10 object-cover bg-white rounded-full"
          />
        </div>
        <div class="ml-2 flex-1">
          ...
          <p class="text-sm">{{ comment.data.attributes.commented_at }}</p>
        </div>
      </div>
    </div>
  </div>
